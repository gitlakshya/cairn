---
name: contextit
description: Generate/maintain the .context/ documentation wiki (init | update). Use when asked to init, update, or refresh the contexIT / context wiki for this repository.
---

# contexIT — documentation agent skill

Invoked as `$contextit` (Codex), via the `skill` tool (opencode), or by asking to "initialize/update the contexit wiki". Same logic as the Claude Code command (`commands/wiki.md`) — kept in sync so behavior does not drift between hosts.
...
- empty / ambiguous → **auto-route**: if `.context/` exists → update, else → init.
  Run: `test -d .context && echo update || echo init`
...
Read `.context/.last-update.json` if it exists, extract `gitHead`.
...
- `status --short` is empty after removing any line whose path is `.context/.last-update.json`;
- HEAD == `gitHead`, **or** every path in `<gitHead>..HEAD` is under `.context/`.
...
Read `.context/.last-update.json` if it exists: extract `gitHead`, `updatedAt`, `status`.
...
for c in \
  ...
  "$HOME/.agents/skills/contextit/scripts/finalize.py" \
  "$HOME/.claude/skills/contextit/scripts/finalize.py"; do
...
Design the smallest complete repository-specific information architecture that helps a coding agent understand and safely change the system. Organize around owned systems, runtime domains, and cross-system workflows — not mirroring the source tree. Use hierarchical paths for meaningful groups: `.context/architecture/`, `.context/concepts/`, `.context/workflows/`, `.context/operations/`, `.context/integrations/`, `.context/testing/` — and top-level `.context/quickstart.md` covering end-to-end state, persistence, and one representative non-obvious file.
...
**Init MUST include `.context/quickstart.md`.** Update MUST NOT delete quickstart. If an update adds, deletes, moves, or materially regroups documentation pages, include `.context/quickstart.md` in the plan so its task-routing map is refreshed. If an update has no required page edits or deletions, plan zero pages.
...
When mode is `init` but `.context/` already exists, treat as fresh generation — existing pages are not preserved; rewrite everything.

**Page count and size constraints:**
- Minimum 1 page (`.context/quickstart.md`), maximum 8 pages total.
...
- `.context/quickstart.md` → **≤ 40 lines total** (including frontmatter). What it is (2–3 sentences), install, one first-run command. Nothing else.
...
If you find an HTML comment starting `"contextit: broken internal link"`, repair the href to restore the target page using the reason in the comment, then delete the comment.
...
Write `.context/.last-update.json` exactly:
...
Get `updatedAt` and `gitHead` via shell (`date -u +%Y-%m-%dT%H:%M:%S.000Z`, `git rev-parse HEAD`) rather than guessing. Write this file on **every** completed run, including no-ops. An interrupted run leaves the previous file untouched so the next update re-runs on the dirty `.context/` tree.
...
**init:**
> Initialize the contexIT wiki for this repository.
> Inspect the project thoroughly, identify major technical and business domains, and write initial documentation under `.context/`.
> Start with `.context/quickstart.md` as the entry point. Then create pages that explain the repository in a way useful to both humans and future agents.
>
> Git context: *(the Step 1 block)*

**update:**
> Update the existing contexIT wiki for this repository.
> Inspect `.context/`, identify recent source changes, and refresh only the documentation pages directly affected by those changes. Use git evidence below when available.
...
> `.context/.last-update.json` is rewritten at the end of every run, including no-ops.
>
> Last update metadata: *(contents of `.context/.last-update.json`, or "No previous contexIT update metadata found.")*

---
description: Generate/maintain .context/ documentation wiki (init | update)
...
- `init` → **init mode**.
- `update` → **update mode**.
- empty → **auto-route**: if `.context/` exists → update, else → init.
...
**Init MUST include `.context/quickstart.md`.** Update MUST NOT delete quickstart. If an update adds, deletes, moves, or materially regroups documentation pages, include `.context/quickstart.md` in the plan so its task-routing map is refreshed. If an update has no required page edits or deletions, submit `pages: []`.
...
**[adapted]** (upstream #699) When mode is `init` but `.context/` already exists, treat as fresh generation — existing pages are not preserved; workers rewrite everything.
...
Get `updatedAt` and `gitHead` via Bash (`date -u +%Y-%m-%dT%H:%M:%S.000Z`, `git rev-parse HEAD`) rather than guessing. Write this file on **every** completed run, including no-ops. Port always writes `complete`: an interrupted run leaves the previous file untouched so the next update re-runs on the dirty `.context/` tree — conservative equivalent to upstream's `interrupted` status.
